...
func NewMessageFilter(interfaceRegistry types.InterfaceRegistry, codec codec.Codec) MessageFilter {
...
func (m MessageFilter) Authenticate(ctx sdk.Context, request AuthenticationRequest) error {
	...
	protoResponseType, err := m.interfaceRegistry.Resolve(request.Msg.TypeURL)
	if err != nil {
		return errorsmod.Wrap(err, "failed to resolve message type")
	}

	// Unmarshal to bytes to the concrete proto message
	err = m.codec.Unmarshal(request.Msg.Value, protoResponseType)
	if err != nil {
	...
	jsonBz, err := m.codec.MarshalInterfaceJSON(protoResponseType)
	if err != nil {
