...
func (m msgServer) AddAuthenticator(
	...
	if err := ctx.EventManager().EmitTypedEvent(&types.AuthenticatorAddedEvent{
		Sender:            msg.Sender,
		AuthenticatorId:   strconv.FormatUint(id, 10),
		AuthenticatorType: msg.AuthenticatorType,
	}); err != nil {
		return nil, errorsmod.Wrapf(err, "failed to emit authenticator added event")
	}
...
func (m msgServer) RemoveAuthenticator(goCtx context.Context, msg *types.MsgRemoveAuthenticator) (*types.MsgRemoveAuthenticatorResponse, error) {
	...
	if err := ctx.EventManager().EmitTypedEvent(&types.AuthenticatorRemovedEvent{
		Sender:          msg.Sender,
		AuthenticatorId: strconv.FormatUint(msg.Id, 10),
	}); err != nil {
		return nil, errorsmod.Wrapf(err, "failed to emit authenticator removed event")
	}
...
func (m msgServer) SetActiveState(goCtx context.Context, msg *types.MsgSetActiveState) (*types.MsgSetActiveStateResponse, error) {
	...
	if err := ctx.EventManager().EmitTypedEvent(&types.AuthenticatorSetActiveStateEvent{
		Sender:               msg.Sender,
		IsSmartAccountActive: msg.Active,
	}); err != nil {
		return nil, errorsmod.Wrapf(err, "failed to emit authenticator state changed event")
	}
