...
func (spkd EmitPubKeyDecoratorEvents) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for i, sig := range sigs {
		signerStrs, err := spkd.ak.AddressCodec().BytesToString(signers[i])
		if err != nil {
			return ctx, errorsmod.Wrap(err, "failed to convert signer address to string")
		}

		events = append(events, sdk.NewEvent(sdk.EventTypeTx,
			sdk.NewAttribute(sdk.AttributeKeyAccountSequence, fmt.Sprintf("%s/%d", signerStrs, sig.Sequence)),
			sdk.NewAttribute(smartaccounttypes.AttributeKeyAccountSequenceAuthenticator, fmt.Sprintf("%s/%d", signerStrs, sig.Sequence)),
		))
