...
import (
	...
	"github.com/ixofoundation/ixo-blockchain/x/nft/exported"
	"github.com/ixofoundation/ixo-blockchain/x/nft/types"
)
...
func (k Keeper) GetNFT(ctx sdk.Context, denom, id string) (nft exported.NFT, err error) {
	...
	if !found {
		return nil, sdk.ErrInternal(fmt.Sprintf("collection of %s doesn't exist", denom))
	}
...
func (k Keeper) UpdateNFT(ctx sdk.Context, denom string, nft exported.NFT) (err error) {
	collection, found := k.GetCollection(ctx, denom)
	if !found {
		return sdk.ErrInternal(fmt.Sprintf("collection #%s doesn't exist", denom))
	}
...
func (k Keeper) DeleteNFT(ctx sdk.Context, denom, id string) (err error) {
	collection, found := k.GetCollection(ctx, denom)
	if !found {
		return sdk.ErrInternal(fmt.Sprintf("collection of %s doesn't exist", denom))
	}
	nft, err := collection.GetNFT(id)
	if err != nil {
		return err
	...
	if !found {
		return sdk.ErrInternal(fmt.Sprintf("id collection #%s doesn't exist for owner %s", denom, nft.GetOwner()))
	}
