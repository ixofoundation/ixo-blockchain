...
func (collection Collection) GetNFT(id string) (nft exported.NFT, err error) {
	...
	return nil, sdk.ErrInternal(fmt.Sprintf("NFT #%s doesn't exist in collection %s", id, collection.Denom))
}
...
func (collection Collection) AddNFT(nft exported.NFT) (Collection, error) {
	...
	if exists {
		return collection, sdk.ErrInternal(fmt.Sprintf("NFT #%s already exists in collection %s", id, collection.Denom))
	}
...
func (collection Collection) UpdateNFT(nft exported.NFT) (Collection, error) {
	...
	if !ok {
		return collection, sdk.ErrInternal(fmt.Sprintf("NFT #%s doesn't exist on collection %s", nft.GetID(), collection.Denom))
	}
