...
func (idCollection IDCollection) DeleteID(id string) (IDCollection, error) {
	index := idCollection.IDs.find(id)
	if index == -1 {
		return idCollection, sdk.ErrInternal(fmt.Sprintf("ID #%s doesn't exist on ID Collection %s", id, idCollection.Denom))
	}
...
func (owner Owner) UpdateIDCollection(idCollection IDCollection) (Owner, error) {
	...
	index := owner.IDCollections.find(denom)
	if index == -1 {
		return owner, sdk.ErrInternal(fmt.Sprintf("ID Collection %s doesn't exist for owner %s", denom, owner.Address))
	}
...
func (owner Owner) DeleteID(denom string, id string) (Owner, error) {
	...
	if !found {
		return owner, sdk.ErrInternal(fmt.Sprintf("ID #%s doesn't exist in ID Collection %s", id, denom))
	}
