...
// Benchmarking :)
// previously using ixomath.Int operations:
// BenchmarkEpochProvision-4 5000000 220 ns/op
//
// using ixomath.Dec operations: (current implementation)
...
func BenchmarkEpochProvision(b *testing.B) {
	...
	r1 := rand.New(s1)
	minter.EpochProvisions = ixomath.NewDec(r1.Int63n(1000000))
...
func TestMinterValidate(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			"negative -errir",
			types.Minter{
				EpochProvisions: ixomath.NewDec(-1),
			},
