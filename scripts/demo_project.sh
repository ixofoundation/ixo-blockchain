...
echo "Funding project at [$PROJECT_ADDR] (using treasury 'oracle-mint' using Francesco oracle)..."
ixocli tx treasury oracle-mint "$PROJECT_ADDR" 10000000000uixo "$FRANCESCO_DID_FULL" "dummy proof" --broadcast-mode block --gas-prices="$GAS_PRICES" -y
...
ixocli tx project create-evaluation "tx_hash" "$SENDER_DID" "claim_id" $STATUS "$PROJECT_DID_FULL" --broadcast-mode block --gas="300000" --gas-prices="$GAS_PRICES" -y
