...
ixocli tx bonds buy 1abc 500res,1000rez "$BOND_DID" "$MIGUEL_DID_FULL" --broadcast-mode block -y
echo "Miguel's account..."
ixocli q auth account "$MIGUEL_ADDR"

echo "Francesco buys 10abc..."
ixocli tx bonds buy 10abc 10100res,10100rez "$BOND_DID" "$FRANCESCO_DID_FULL" --broadcast-mode block -y
echo "Francesco's account..."
ixocli q auth account "$FRANCESCO_ADDR"

echo "Miguel swap 500 res to rez..."
ixocli tx bonds swap 500 res rez "$BOND_DID" "$MIGUEL_DID_FULL" --broadcast-mode block -y
echo "Miguel's account..."
ixocli q auth account "$MIGUEL_ADDR"

echo "Francesco swap 500 rez to res..."
ixocli tx bonds swap 500 rez res "$BOND_DID" "$FRANCESCO_DID_FULL" --broadcast-mode block -y
echo "Francesco's account..."
ixocli q auth account "$FRANCESCO_ADDR"

echo "Miguel swaps above order limit (tx will fail)..."
ixocli tx bonds swap 5001 res rez "$BOND_DID" "$MIGUEL_DID_FULL" --broadcast-mode block -y
echo "Miguel's account (no  changes)..."
ixocli q auth account "$MIGUEL_ADDR"

echo "Francesco swaps to violate sanity (tx will be successful but order will fail)..."
ixocli tx bonds swap 5000 rez res "$BOND_DID" "$FRANCESCO_DID_FULL" --broadcast-mode block -y
echo "Francesco's account (no changes)..."
ixocli q auth account "$FRANCESCO_ADDR"

echo "Miguel sells 1abc..."
ixocli tx bonds sell 1abc "$BOND_DID" "$MIGUEL_DID_FULL" --broadcast-mode block -y
echo "Miguel's account..."
ixocli q auth account "$MIGUEL_ADDR"

echo "Francesco sells 10abc..."
ixocli tx bonds sell 10abc "$BOND_DID" "$FRANCESCO_DID_FULL" --broadcast-mode block -y
echo "Francesco's account..."
ixocli q auth account "$FRANCESCO_ADDR"
