...
tx_from_m buy 1abc 500res,1000rez U7GK8p8rVhJMKhBVRCJJ8c "$MIGUEL_DID"
echo "Miguel's account..."
ixocli query auth account "$MIGUEL_ADDR"

echo "Francesco buys 10abc..."
tx_from_f buy 10abc 10100res,10100rez U7GK8p8rVhJMKhBVRCJJ8c "$FRANCESCO_DID"
echo "Francesco's account..."
ixocli query auth account "$FRANCESCO_ADDR"

echo "Miguel swap 500 res to rez..."
tx_from_m swap 500 res rez U7GK8p8rVhJMKhBVRCJJ8c "$MIGUEL_DID"
echo "Miguel's account..."
ixocli query auth account "$MIGUEL_ADDR"

echo "Francesco swap 500 rez to res..."
tx_from_f swap 500 rez res U7GK8p8rVhJMKhBVRCJJ8c "$FRANCESCO_DID"
echo "Francesco's account..."
ixocli query auth account "$FRANCESCO_ADDR"

echo "Miguel swaps above order limit..."
tx_from_m swap 5001 res rez U7GK8p8rVhJMKhBVRCJJ8c "$MIGUEL_DID"
echo "Miguel's account (no  changes)..."
ixocli query auth account "$MIGUEL_ADDR"

echo "Francesco swaps to violate sanity..."
tx_from_f swap 5000 rez res U7GK8p8rVhJMKhBVRCJJ8c "$FRANCESCO_DID"
echo "Francesco's account (no changes)..."
ixocli query auth account "$FRANCESCO_ADDR"

echo "Miguel sells 1abc..."
tx_from_m sell 1abc U7GK8p8rVhJMKhBVRCJJ8c "$MIGUEL_DID"
echo "Miguel's account..."
ixocli query auth account "$MIGUEL_ADDR"

echo "Francesco sells 10abc..."
tx_from_f sell 10abc U7GK8p8rVhJMKhBVRCJJ8c "$FRANCESCO_DID"
echo "Francesco's account..."
ixocli query auth account "$FRANCESCO_ADDR"
